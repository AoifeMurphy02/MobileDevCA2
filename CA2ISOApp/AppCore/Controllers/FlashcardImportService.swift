...
enum FlashcardImportService {
    ...
    private nonisolated static func buildLocalDraftDeck(
        ...
        let metadataSuggestion = FlashcardAIEnhancer.suggestMetadata(
            ...
            fallbackTitle: title,
            availablestudySubjects: availablestudySubjects,
            preferredstudySubject: preferredstudySubject.isEmpty ? studySubject : preferredstudySubject,
            fallbackTopic: topic
        )
        ...
        return FlashcardDeckDraft(
            ...
            sourceType: sourceType,
            studySubject: metadataSuggestion.studySubject,
    ...
    nonisolated static func buildSet(from draftDeck: FlashcardDeckDraft) throws -> FlashcardSet {
        ...
        let resolvedstudySubject = draftDeck.studySubject.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private nonisolated static func mergeCloudSuggestion(
        ...
        return FlashcardDeckDraft(
            ...
            studySubject: preferredMetadataValue(suggestion.studySubject, fallback: localDraftDeck.studySubject),
        ...
        for section in selectedSections {
            ...
            requests.append(
                FlashcardCloudGenerationRequest(
                    ...
                    studySubject: localDraftDeck.studySubject,
    ...
    private nonisolated static func combinedCloudSuggestion(
        ...
        var resolvedstudySubject = fallbackDeck.studySubject
        var resolvedTopic = fallbackDeck.topic

        for suggestion in suggestions {
            resolvedTitle = preferredMetadataValue(suggestion.title, fallback: resolvedTitle)
            resolvedstudySubject = preferredMetadataValue(suggestion.studySubject, fallback: resolvedstudySubject)
    ...
    private nonisolated static func makePurposeCard(from sentence: String) -> FlashcardDraft? {
        ...
        for pattern in patterns {
            ...
            guard let match = regex.firstMatch(in: sentence, options: [], range: range),
                  match.numberOfRanges == 3,
                  let studySubjectRange = Range(match.range(at: 1), in: sentence),
                  let remainderRange = Range(match.range(at: 2), in: sentence) else {
                continue
            }

            let studySubject = sentence[studySubjectRange].trimmingCharacters(in: .whitespacesAndNewlines)
            let remainder = sentence[remainderRange].trimmingCharacters(in: .whitespacesAndNewlines)

            guard isReasonablePrompt(studySubject), isSpecificStudyConcept(studySubject), remainder.count > 8 else { continue }

            return FlashcardDraft(
                question: "What is \(studySubject) used for?",
