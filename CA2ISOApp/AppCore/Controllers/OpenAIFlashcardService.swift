...
struct FlashcardCloudDeckSuggestion: Sendable {
    ...
    nonisolated init(title: String, subject: String, topic: String, cards: [FlashcardCloudCardSuggestion]) {
        self.title = title
        self.subject = subject
        self.topic = topic
        self.cards = cards
...
private struct OpenAIFlashcardProvider: FlashcardLLMProvider {
    ...
    private nonisolated func groundedDeckPrompt(for request: FlashcardCloudGenerationRequest) -> String {
        ...
        - Subject: \(fallbackValue(request.subject, placeholder: "Use the source"))
    ...
    private nonisolated func deckSuggestionSchema() -> [String: Any] {
        [
            ...
            "required": ["title", "subject", "topic", "cards"]
        ]
    }
    ...
    private nonisolated func parseDeckPayload(from data: Data) throws -> CloudDeckPayload {
        ...
        return CloudDeckPayload(title: title, subject: subject, topic: topic, cards: cards)
    }
...
private struct CloudDeckPayload: Sendable {
    ...
    nonisolated init(title: String, subject: String, topic: String, cards: [CloudDeckCardPayload]) {
        self.title = title
        self.subject = subject
        self.topic = topic
        self.cards = cards
